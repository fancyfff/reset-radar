...
> **如何判断「数据源」是否真正成功（不只是流程成功）**
> - `data.json` 有更新 / 出现自动 commit，只能说明**脚本跑通**，不足以证明抓取成功。
> - 抓取失败时会**静默回退种子**，但照样写盘、照样提交 → 会出现「`last_updated` 更新了，数值却来自种子/旧值」的假象。
> - 因此判断数据源成功，必须看具体某次 run 的 `运行采集脚本` 日志：
>   - 有 `[warn] 抓取失败 ...` → 该源已降级（种子/回退数据）。
>   - 无 `[warn]`/`[err]` → 该源真实抓取成功。
>   - 警告按平台独立：某平台可能只有 GitHub 或 Status 之一降级。

> **降级保护（防止"静默假更新"）**
> - 每平台带 `data_source`（live / seed），seed（回退数据）平台会在页面显示橙色徽标，明确告知并非实时抓取。
> - 若本轮**所有平台均未抓到任何真实源**（全降级），引擎**不写 `data.json` / `fallback.js`、也不产生自动 commit**，保留上次真实数据，日志打印 `[warn] … 全降级 …`，可由 `git-auto-commit` 因数据无变化而不提交。
